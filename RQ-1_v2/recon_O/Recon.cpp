...
Recon::Recon()
  ...
  // Update these to your ESP32 GPIO pins
  board_pins[FRONT_RIGHT_HIP] = 13;  // Changed from 2
  board_pins[FRONT_LEFT_HIP] = 12;   // Changed from 8
  board_pins[BACK_RIGHT_HIP] = 14;   // Changed from 4
  board_pins[BACK_LEFT_HIP] = 27;    // Changed from 6
  board_pins[FRONT_RIGHT_LEG] = 26;  // Changed from 3
  board_pins[FRONT_LEFT_LEG] = 25;   // Changed from 9
  board_pins[BACK_RIGHT_LEG] = 33;   // Changed from 5
  board_pins[BACK_LEFT_LEG] = 32;    // Changed from 7
}
...
void Recon::init() {
  ...
  ledc_timer_config_t timer_conf = {
      ...
      .freq_hz = 50,  // Standard servo PWM frequency
  ...
  for (int i = 0; i < 8; i++) {
    ledc_channel_config_t channel_conf = {
        .gpio_num = board_pins[i],
        .speed_mode = LEDC_MODE,
        .channel = (ledc_channel_t)(LEDC_CHANNEL_START + i),
...
void Recon::setServo(int id, float target) {
  // Convert angle to duty cycle (ESP32 uses 0-8191 for 13-bit resolution)
  float angle = reverse[id] ? (180 - (target + trim[id])) : (target + trim[id]);
  uint32_t duty = (angle / 180.0) * 4095;  // For 12-bit resolution
  
  // Set duty cycle
  ledc_set_duty(LEDC_MODE, (ledc_channel_t)(LEDC_CHANNEL_START + id), duty);
  ledc_update_duty(LEDC_MODE, (ledc_channel_t)(LEDC_CHANNEL_START + id));
